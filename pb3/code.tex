...
\subsection*{4. Algorithm Complexity:}
From the hypothesis it is specified that $max(c(e))=\mathcal{O}(1)$. We denote this constant by $k$.
\begin{itemize}
\item Computing $\alpha$:
\begin{itemize}
\item Traversing the edges: $\mathcal{O}(m)$.
\item Sorting the unique capacities: $\mathcal{O}(k\times \log k)$, but $k$ is constant so in the end it remains $\mathcal{O}(1)$.
\item Total cost: $\mathcal{O}(m)$.
\end{itemize}
\item Computing the maximum flow (Ford–Fulkerson algorithm):
\begin{itemize}
\item Cost of each iteration: Each increase of the current flow requires at most $2m$ inspections of arcs for labeling other nodes, therefore $\mathcal{O}(m)$.
\item Number of iterations: If all capacities are integers, at most $v$ augmentations are needed ($v$ is the value of the maximum flow), thus $\mathcal{O}(mv)$.
\item Flow upper bound: If $U$ is an upper bound of all arc capacities, then $v \le (n-1)U$ (this is an upper bound of the cut $({s}, V \setminus {s})$), therefore $\mathcal{O}(nmU)$.
\item In the hypothesis it is stated that the capacity function is non-constant but bounded by a constant. Thus $U$ is a constant that does not depend on $n$ or $m$.
\item Total cost: $\mathcal{O}(nm)$.
\end{itemize}
\item Final complexity: $\mathcal{O}(nm)$
\end{itemize}

\subsection*{5. The Algorithm (pseudocode):}
\textbf{procedure} CheckCriticalEdge($G, c, s, t, e_0$)
\begin{tabbing}
\hspace{0.5cm} \= \hspace{0.5cm} \= \hspace{0.5cm} \= \kill
\> // cap is the vector of capacities \\
\>sort(cap) \\
\>$\alpha \leftarrow \infty$ \\
\> \textbf{for} ($i = 1, i < k$) \textbf{do} \\
\> \> \textbf{if} ($cap_{i+1} - cap_i < \alpha$) \textbf{then} \\
\> \> \> $\alpha \leftarrow cap_{i+1} - cap_i$ \\
\> \> \textbf{end if} \\
\> \textbf{end for} \\
\\
\> \textbf{if} ($\alpha = \infty$) \textbf{then} $\alpha \leftarrow 1$ \textbf{end if} \\
\\
\> // MaxFlow is the Ford–Fulkerson algorithm \\
\> $f_{old\_max} \leftarrow$ MaxFlow($G, s, t, c$) \\
\\
\> $c(e_0) \leftarrow c(e_0) + \alpha$ \\
\\
\> $f_{new\_max} \leftarrow$ MaxFlow($G, s, t, c$) \\
\\
\> \textbf{if} ($f_{new\_max} > f_{old\_max}$) \textbf{then} \\
